...
{
	ASSERT(H_image.GetChannel() == 1);
	ASSERT(S_image.GetChannel() == 1);
	ASSERT(V_image.GetChannel() == 1);

	int nWidth = H_image.GetWidth();
	int nHeight = H_image.GetHeight();

	for (int r = 0; r<nHeight; r++)
	{
		BYTE* pHIn = H_image.GetPtr(r);
		BYTE* pSIn = S_image.GetPtr(r);
		BYTE* pVIn = V_image.GetPtr(r);

		BYTE* pOut = imageOut.GetPtr(r);

		for (int c = 0; c<nWidth; c++)
		{
			if(pHIn[c] == 255 && pSIn[c] == 255 && pVIn[c] == 255)
				pOut[c] = 255;
			else 
				pOut[c] = 0;
