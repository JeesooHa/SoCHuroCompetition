...
void draw_rectfill2(U8* in, U8* out,int mode)
{
	int x=0, y=0;
	int pos = 0;

	draw_rectfill(0, 0, 10, 10, MAKE_COLORREF(255, 0, 0));
	draw_rectfill(320 -10, 0, 10, 10, MAKE_COLORREF(0, 255, 0));

	EGL_COLOR clr = MAKE_COLORREF(255, 255, 255);
	
	if(mode == C_YELLO || mode == C_GATE)	clr = MAKE_COLORREF(255, 255, 0);	//YELLO
	else if (mode == C_RED)		clr = MAKE_COLORREF(255, 0, 0);		//RED
	else if (mode == C_BALL)	clr = MAKE_COLORREF(255, 50, 0);	//BALL
	else if (mode == C_GOAL)	clr = MAKE_COLORREF(150, 100, 0);	//MDF
	else if (mode == C_GREEN)	clr = MAKE_COLORREF(0, 255, 0);		//GREEN
	else if (mode == C_BLK)		clr = MAKE_COLORREF(0, 255, 255);		//BLACK
	else if (mode == C_BLUE)	clr = MAKE_COLORREF(0, 0, 255);		//BLUE
	...
	for (y = 0; y < HEIGHT; y++){
		for (x = 0; x < WIDTH; x++) {
			...
			if(in[pos] == 255) {	
				//out[4* pos + 4 * j + i] = 255;
				//draw_rectfill(1.77*(4 * x + i), 250 + 1.77*(4 * y + j), 2, 2, MAKE_COLORREF(150, 100,0));	
				draw_rectfill(320-2.66*(M_SIZE * y )-5, 2.66*(M_SIZE * x )-5, 11, 11, clr);
			}
